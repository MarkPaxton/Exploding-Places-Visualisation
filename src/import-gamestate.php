...
try
{
	...
	$file = fopen('./GameStateDrossq_full.txt', 'r');
	$line_count=0;
	while($line = fgets($file))
	{
		...
		{
			$value_items[] = current($line_parts);	
		}
		
		$value = $dbh->quote(implode(';', str_replace("\n", "", $value_items)));
		
		$query = "INSERT INTO gamestate (timestamp, action, value, user) VALUES ('$timestamp', $action, $value, 'Drossq');";
 		echo($query . "\n");
